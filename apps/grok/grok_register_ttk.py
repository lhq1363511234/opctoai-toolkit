...
try:
    ...
    # Web/CLI headless path does not need a desktop GUI toolkit.
    tk = None  # type: ignore
...
def _append_sso_pending(email: str, sso: str, log_callback=None):
    ...
    try:
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sso_pending.txt")
        line = f"{email}----{sso}\n" if email else f"{sso}\n"
...
class GrokRegisterGUI:
    ...
    def _on_check_done(self, text, all_ok):
        self.check_btn.config(state=None)
    ...
    def _set_running_ui(self, running):
        ...
        self.stop_btn.config(state=None if running else tk.DISABLED)
...
def run_registration_cli(count):
    # Web/CLI 共用入口：GUI 在 main() 里 wire，这里也必须注入 deps，
    # 否则 register_flow.fill_email_and_submit 会 KeyError: get_email_and_token
    _wire_runtime_modules()
    ...
    try:
        ...
        try:
            ...
            if user_stopped and not should_close_browser_after_run(True):
                ...
                # 正常结束 or 用户停止且要求关闭：统一做线程关闭 + 残留清理
                reason = "用户停止" if user_stopped else "任务结束"
                cleanup_runtime_memory(log_callback=cli_log, reason=reason)
        except BaseException:
            pass
        try:
            cli_log(
...
def main():
    if not TK_AVAILABLE:
        raise SystemExit(
            ...
            "Web/CLI 模式不需要 GUI，请使用 web_app.py 或 register_cli.py。"
        )
    try:
    ...
    if not TK_AVAILABLE:
        raise SystemExit(
            "当前 Python 环境未安装 tkinter，无法启动桌面 GUI。\n"
            "Web/CLI 模式不需要 GUI，请使用 web_app.py 或: python grok_register_ttk.py cli"
        )
    root = tk.Tk()
