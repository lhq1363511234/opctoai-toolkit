...
def _patched_create_browser_options():
    ...
    # Prefer our Playwright Chrome for Testing symlink; fall back to system chrome.
    browser_set = False
    for cand in (
        ...
        if os.path.isfile(cand) or os.path.islink(cand):
            ...
            break
    if not browser_set:
        print("[warn] no chromium binary found")

    # Stability flags for headless/Xvfb Linux + root
    for flag in (
        "--no-sandbox",
    ...
    # If no real DISPLAY / Xvfb, force new headless
    ...
    if not display:
        ...
        # Xvfb present: still avoid crashy GPU paths
        try:
            opts.set_argument("--ozone-platform=x11")
        except Exception:
            pass

    ext_path = os.path.join(os.path.dirname(os.path.abspath(reg.__file__)), "turnstilePatch")
    # Extensions often break headless/Xvfb cold-start; only load when DISPLAY set
    if os.path.isdir(ext_path) and display:
        try:
